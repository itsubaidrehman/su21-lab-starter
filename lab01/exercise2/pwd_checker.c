...
bool check_length(const char *password) {
    ...
    bool meets_len_req = (length >= 10); // Previously length logic was length should be less than or equal to 10 but we want len should be atleat 10 
...
// while debugging check_upper func, i came to know that there is logical error in check_range 
// As the test password contains A that is the first upper case Alphabet and the logic says that letter > lower so it will never detect A, a or 0
...
bool check_name(const char *first_name, const char *last_name, const char *password) {
    ...
    // since strstr returns 0 if the 2nd argument is not present in the 1st argument if first is 0 that means name is not found
    // so we need to change return logic
